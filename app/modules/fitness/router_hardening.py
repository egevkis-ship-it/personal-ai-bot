...
async def _handle_cancel_planned_confirmation(telegram_user_id: str | None, text: str, pending: dict) -> str | None:
    ...
    if decision == "unknown":
        # Если пользователь явно переключается на другой запрос (показ, старт
        # тренировки, запись подходов, и т.п.) — авто-отменяем pending,
        # чтобы не блокировать его всю жизнь.
        t_low = (text or "").lower()
        if any(p in t_low for p in [
            "покажи", "что у меня", "что сегодня", "что завтра", "что вчера",
            "что в плане", "план на", "начинаю", "начал", "приступаю",
            "стартую", "запусти", "/start", "/today", "/help", "/reset",
            "сделал", "выполнил", "записать", "записываем",
        ]):
            await resolve_fitness_pending_decision(pending["id"], status="cancelled")
            return None  # пусть основной диспетчер обработает запрос
        return (
...
def _is_show_active_workout_log_request(text: str | None) -> bool:
    ...
    # Если в тексте есть дата/день недели — НЕ показываем активную сессию,
    # пользователь спрашивает про конкретный день, не про текущий лог.
    has_date_qualifier = any(d in t for d in [
        "сегодня", "вчера", "завтра", "позавчера",
        "на пн", "на вт", "на ср", "на чт", "на пт", "на сб", "на вс",
        "в понедельник", "во вторник", "в среду", "в четверг", "в пятницу", "в субботу", "в воскресенье",
        "понедельник", "вторник", "среда", "четверг", "пятница", "суббота", "воскресенье",
        "на след", "на этой неделе", "на следующей", "на прошлой",
        "числа", "число", "мая", "июня", "июля", "марта", "апреля", "августа",
    ])
    if has_date_qualifier:
        return False

    return any(x in t for x in (
        "покажи текущую тренировку",
        "текущая тренировка",
        "покажи активную тренировку",
        "покажи активную сесси",
        "активную сесси",
        "что в активной сесси",
        "что у меня в активной",
        "покажи мою сесси",
        "покажи мою тренировку",
        "что я уже сделал",
        "что уже сделал",
        "что сделал",
        "покажи что я сделал",
...
async def handle_router_hardening(telegram_user_id: str | None, text: str) -> str | None:
    ...
    _archive_verb = any(v in _t_arch for v in [
        ...
        "что я тренировал", "что выполнил", "что записал",
    ])
    _archive_time = any(t in _t_arch for t in [
