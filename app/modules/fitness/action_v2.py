...
ФАКТ (show_completed_*) — пользователь спрашивает про ВЫПОЛНЕННОЕ:
- "что я делал сегодня/вчера/15-го" → show_completed_day
- "что я сделал на этой неделе" → show_completed_week
- "что я сделал в этом месяце" → show_completed_month
Маркер ФАКТА: "я делал" / "я сделал" / "я записал" / "я зафиксировал" / "что было" + временной маркер.

ПЛАН (show_*_workout / show_*_plan) — пользователь спрашивает про ЗАПЛАНИРОВАННОЕ:
- "что у меня сегодня по плану" / "что сегодня" → show_today_workout
- "план на неделю" → show_week_plan
Маркер ПЛАНА: "у меня" / "по плану" / "в плане" / "план на".

Если сомневаешься — выбирай show_completed_* для прошлого времени и show_*_workout/plan для настоящего/будущего.
